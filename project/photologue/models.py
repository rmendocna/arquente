...
class Photo(ImageModel):
    ...
    title_slug = models.CharField(_('slug'), max_length=255, unique=True, blank=True,
                                  help_text=('A "slug" is a unique URL-friendly title for an object.'))
    caption = models.TextField(_('caption'), blank=True)
    date_added = models.DateTimeField(_('date added'), default=datetime.now, editable=False)
    is_public = models.BooleanField(_('is public'), default=True, help_text=_('Public photographs will be displayed in the default views.'))
    is_highlight = models.BooleanField(_('is highlighted'), default=True, help_text=_('Highlighted images will be promoted to home page'))
...
class PhotoEffect(BaseEffect):
    ...
    tint = models.CharField(_('tint'), max_length=30, blank=True, help_text=_("You can either enter an RGB code in the form #HHHHHH or an accepted color name."))
    ...
    def post_process(self, im):
        ...
        #if self.tint not in ['',None]:
        #    im = ImageChops.multiply(im, Image.new('RGB', im.size, self.tint))
        return im
...
import widgets as photo_widgets
...
class ManyToManyPhotosField(models.ManyToManyField):
    ...
    def formfield(self, **kwargs):
        defaults = {'widget': photo_widgets.ManyToManyImages(self.verbose_name,False)}
        defaults.update(kwargs)
        defaults['widget'] = photo_widgets.AdminManyToManyPhotosWidget(self.verbose_name,False)
